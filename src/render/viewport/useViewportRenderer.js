...
export function useViewportRenderer({
  ...
  // world zoom (2/3/4...) - keep integer for crisp pixel art
  zoom = 4,

  // pixel-art mode (turns off smoothing + snaps camera)
  pixelArt = true,
}) {
  useEffect(() => {
    ...
    function updateCamera(dt) {
      ...
      // Lock camera exactly to target once it's within half a screen-pixel.
      // Prevents the exponential chase from oscillating around the pixel-snap
      // grid when the player is standing still (the root cause of sprite jiggle).
      const snapEps = pixelArt && z > 1 ? 0.5 / z : 0.01;
      if (Math.abs(smooth.x - tx) < snapEps) smooth.x = tx;
      if (Math.abs(smooth.y - ty) < snapEps) smooth.y = ty;

      // Pixel-art camera snap: round to 1/z world units so that after
      // scaling by z we land on whole screen pixels.
      if (pixelArt && z > 1) {
